...
# Suggested Dedicated Lamp COlumn Charge Pints
area1 <- read_sf("../../LocationAllocation/area1_ded_v2.geojson")
area2 <- read_sf("../../LocationAllocation/area2_ded_v2.geojson")
area3 <- read_sf("../../LocationAllocation/area3_ded_v2.geojson")
ded_sugg <- bind_rows(area1, area2, area3) %>%
...
lccp <- read_excel("../../../data/clean_data_files/charger_locations/2021 - 04 Electric Charge Point Master Asset List.xlsx", skip=1)
lccp <- lccp %>%
  dplyr::filter(!`L-EV Charge Point ID` %in% ded_sugg$ChargepointID) %>%
...
leaflet() %>%
  ...
  addCircleMarkers(data=ded_sugg, stroke=F, radius = 3, fillOpacity = 1, fillColor = "blue", group="LCCP To Dedicate", label=~paste(ChargepointID, LocationName, FeatureLocation, sep=" : ")) %>%
  ...
  addLayersControl(overlayGroups = c("LCCP Not Dedicated", "LCCP To Dedicate", "Existing Dedicated", "Bay Lines"), baseGroups = c("Map", "Satellite"), options=layersControlOptions(collapsed = FALSE)) %>%
...
leaflet() %>%
  ...
  #addCircleMarkers(data=existing_dedicated_ward, stroke=F, radius = 3, fillOpacity = 1, fillColor = "red", group="Existing Dedicated", label=~paste(L.EV.Charge.Point.ID, Site.Name, Feat.Loc, sep=" : ")) %>%
    addLayersControl(overlayGroups = c("LCCP Not Dedicated", "LCCP To Dedicate", "Existing Dedicated", "Bay Lines"), baseGroups = c("Map", "Satellite"), options=layersControlOptions(collapsed = FALSE)) %>%
    addMeasure(primaryLengthUnit = "meters", secondaryLengthUnit = "feet", position="top;eft") %>%
